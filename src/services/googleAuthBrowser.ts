...
export function getBrowserAuthConfigError(): string | null {
  ...
  if (typeof id !== 'string' || id.length === 0) {
    return 'VITE_GOOGLE_WEB_CLIENT_ID is missing or empty in .env.local. Restart the dev server after adding it.'
  }
...
export function buildLoginUrl(): string {
  ...
  if (!config) {
    throw new Error(
      err ??
        'Web OAuth not configured. Add VITE_GOOGLE_WEB_CLIENT_ID and VITE_GOOGLE_WEB_CLIENT_SECRET to .env.local. See GOOGLE_OAUTH_SETUP.md.'
    )
  }
